...

{
    ...
    PAGED_CODE();
    // this should be the IOCTL handler, according to the book, the thread I create here should be running in the context of
    // the calling process, which means the thread will be running in user mode, let's test it
    TestFunction();
    //HANDLE pid = PsGetCurrentProcessId();
    //PEPROCESS process;
    //NTSTATUS status = PsLookupProcessByProcessId(pid, &process);
    //KdPrint(("error code: %d\n", status));
    //TestFunction();
    HANDLE thread_handle;
    NTSTATUS ret = PsCreateSystemThread(&thread_handle, GENERIC_ALL, NULL, NULL, NULL, thread_routine, NULL);
    KdPrint(("error code: %d\n", ret));
    TestFunction();
